    ...
    for (uint32_t i = 0; i < graph->node_count; i++) {
        ...
        if (node->op_name != NULL) {
            for (uint32_t i = 0; i < node->output_count; i++) {
                ...
                connx_intermediate_write(fp, &tensor->dtype, sizeof(uint32_t));
                connx_intermediate_write(fp, &tensor->ndim, sizeof(uint32_t));
                connx_intermediate_write(fp, tensor->shape, sizeof(uint32_t) * tensor->ndim);
                connx_intermediate_write(fp, tensor->buffer, tensor->size);
                fclose(fp);
            }
        }
#endif
